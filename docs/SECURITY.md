...
| **Auto-listen zones are owner opt-in** | Without `!listen`, non-mention channel messages are never processed. Inside a zone, guests still run read-only + rate-limited, `!block` overrides the zone, @mentions aren't double-answered, and bot-authored messages are ignored — no bot-to-bot loops. |
