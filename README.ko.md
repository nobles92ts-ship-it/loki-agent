...
`{args}` 자리에 인자가 들어가고, 없으면 뒤에 붙는다. 별칭은 *프롬프트*지 새 권한이 아니다 — 그 문장을 직접 친 것과 똑같이 동작하므로 큐·throttle·게스트 범위가 그대로 적용되고, 기본 명령을 가로챌 수 없다. 오너는 모든 별칭을, 게스트는 소속 조직이 허용받은 것만 실행할 수 있다(`!org allow acme 스탠드업`). 프롬프트가 아니라 실제 코드가 필요한 파이프라인은 여전히 private-command 훅이 답이다 — [docs/EXAMPLES.md](docs/EXAMPLES.md) 참고.
